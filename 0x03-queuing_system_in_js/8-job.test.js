...
describe('createPushNotificationsJobs', () => {
    ...
    it('display a error message if job is not an array', () => {
    ...
    it('throws an error if queue is not a value Kue ', function () {
    ...
    it('test job progress event report', (done) => {
        ...
        queue.testMode.jobs[0].addListener('progress', () => {
            ...
            expect(cosole.log.calledWith(`Notification job ${id} 50% complete`)).to.be.true;
            done();    
        });
    ...
    it('test job complete event report', (done) => {
        createPushNotificationsJobs(jobs, queue);
        queue.testMode.jobs[0].addListener('complete', () => {
            const id = queue.testMode.jobs[0].id;
            expect(cosole.log.calledWith(`Notification job ${id} failed: completed`)).to.be.true;
            done();    
        });
